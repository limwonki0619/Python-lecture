...
## 아나콘다와 파이참 가상환경 연동시키기
 ...
 > 3. 그럼 기존의 `base(root)` 아래에 사용자가 직접 이름지은 가상환경이 만들어집니다.
 > + *추가적으로 `Search Packages`로 필요한 패키지를 선택하여 설치할 수 있습니다.*
 > 4. 가상환경이 만들어지면 **파이참**으로 가서 **File** -> **Close Project**로 기존의 프로젝트롤 종료해주고 
 > 5. `Create New Project`를 클릭해 새로운 프로젝트를 만들 준비를 합니다.
 > 6. 이후 **Location**에서 새로운 프로젝트의 폴더명을 만들어 준 후 아래에 있는 `Project Interpreter 부분의 화살표`를 눌러줍니다.
 > 7. 여기서 `Existing interpreter`를 눌러 **Anaconda에서 만들어준 가상환경 폴더에 있는 Python**을 선택해 주고, `Create`를 눌러줍니다.
 > 8. 새로운 환경에서 `test.py`를 생성한 후 아래에 있는 `Console`창에서 파이썬 버전을 확인하고,`hello world`를 출력해 봅니다.
 > 9. 출력이 정상적으로 된다면 가상환경 구축에 성공 한 겁니다.
<br>
